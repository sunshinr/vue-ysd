  ...
  <div class="edit-schema-template-wrap">
    ...
    <div class="edit-schema-template-main">
      ...
      <div class="edit-schema-template-top" id="cycle">
        <label>方案周期</label><input type="text" placeholder="0" v-model="schemeCycle" v-verify="schemeCycle"> <span>天</span>
      </div>
      ...
      <!--标语-->
      <textarea class="tips" v-model="schemeDesc" v-verify="schemeDesc" placeholder="在此添加描述"></textarea>
    </div>
      ...
      async subButton () {
        if(this.$verify.check()){
          ...
          for(var i = 0 ; i< this.newschemeRecipesVos.length; i++) {
            ...
            for(var n = 0; n < this.newschemeRecipesVos[i].schemeRecipesInfos.length; n++){
              for(var p = 0; p < this.newschemeRecipesVos[i].schemeRecipesInfos[n].data.length; p++){
                // cxcx.push(this.newschemeRecipesVos[i][n].data[p])
          ...
          let params = {
            ...
            sportItem: JSON.stringify(this.schemeModelSportVos),
            recipes:  JSON.stringify(foodlist),
          }
          const res = await api.post('doctorScheme/saveSchemeModel.do', params)
          if (res.data.stateCode === '001') {
            let instance = Toast({
              message: '保存成功,返回',
              position: 'middle',
