<template>
  <div class="add-evaluation-second-wrap">
    <HeaderBar3 title="添加评估"></HeaderBar3>
      ...
      <mt-tab-container v-model="selected">
        <mt-tab-container-item id="0">
          ...
          <div class="add-evaluation-second">
            <div>
              <h3>标题</h3>
              <input type="text" placeholder="请在此输入问题标题" v-model="questionTitle" v-focus>
            </div>
            <div>
              <h3>选项</h3>
              <div v-for="(value,index) of num" :key="value.id" class="option_box">
                <input placeholder="请输入选项内容" v-model="num[index].content">
                <span class="font iconfont icon-shanchu-" @click="deleteOption(index, 1)"></span>
              </div>
              <p v-show="hintSingle"><i class="font iconfont icon-gantanhao--"></i>请至少保留两个选项！</p>
              <button type="button" @click="addOption(1)">添加选项</button>
            </div>
          </div>
        ...
        <mt-tab-container-item id="1">
          ...
          <div class="add-evaluation-second">
            ...
            <div>
              ...
              <div v-for="(value,index) of num" :key="value.id" class="option_box">
                <input placeholder="请输入选项内容" v-model="num[index].content">
                <span class="font iconfont icon-shanchu-" @click="deleteOption(index, 2)"></span>
              </div>
              <p v-show="hintMultiple"><i class="font iconfont icon-gantanhao--"></i>请至少保留两个选项！</p>
              <button type="button" @click="addOption(2)">添加选项</button>
        ...
        <mt-tab-container-item id="2">
          ...
          <div class="add-evaluation-second">
            <div>
              <h3>标题</h3>
              <input type="text" placeholder="请在此输入问题标题" v-model="questionTitle">
              <h3 class="add-evaluation-second-choose">回答最多输入<input type="text" placeholder="1" v-model="least"> 字</h3>
            </div>
          </div>
        </mt-tab-container-item>
      </mt-tab-container>
    ...
    methods: {
      addOption(amount) {
        ...
        if (this.num.length>2) {
          if (amount == 1) {
            this.hintSingle = false
          } else {
            this.hintMultiple = false
          }
        }
      },
      deleteOption(index, amount) {
        if(this.num.length>2){
        console.log(index);
        ...
        console.log(this.num);
          ...
          if (amount == 1) {
            this.hintSingle = true
          } else {
            this.hintMultiple = true
          }
        }
      },
      optionSort() {
      ...
      tipNumchangeBg(num) {
        ...
        $(".tip-wrap li").eq(num - 1).addClass("tip-active");
        this.tipnum = num
        //console.log(this.evaluateQuestionList);
